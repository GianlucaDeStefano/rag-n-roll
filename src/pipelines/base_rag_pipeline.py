    ...
    def run(self,query_id, question, related_doc_ids = []): 
        ...
        for doc in documents:
            ...
            if related_doc_ids and not doc.metadata['is_irrelevant']:
                assert doc.metadata['id'] in related_doc_ids, f"Retrieved relevant document with id: {doc.metadata['id']}, but it is not in the allowed documents list: {related_doc_ids}"

            if self.num_benign_docs is not None and not doc.metadata['is_adversarial']:
                assert int(doc.metadata['counter']) < self.num_benign_docs, f"Retrieved benign document with id: {doc.metadata['id']} has conter > {self.num_benign_docs}"
            
            if self.num_malicious_docs is not None and doc.metadata['is_adversarial']:
                assert int(doc.metadata['counter']) < self.num_malicious_docs, f"Retrieved adversarial document with id: {doc.metadata['id']} has counter {doc.metadata['counter']} >= {self.num_malicious_docs}"
            ...
            if doc.metadata['is_adversarial']: 
                ...
                assert doc.metadata['id'] in related_doc_ids, f"Adversarial document found: {doc.metadata} but is not in the related documents list: {related_doc_ids}"
                mal_doc_ids += [doc.metadata['id']]
                
            if not doc.metadata['is_irrelevant'] and not doc.metadata['is_adversarial']:
                assert doc.metadata['id'] in related_doc_ids, f"Benign document found: {doc.metadata} but is not in the related documents list: {related_doc_ids}"
                ben_doc_ids += [doc.metadata['id']]
...
def make_model(model_config): 
    ...
    if model_type == 'openai':
        return ChatOpenAI(name=model_config.name, temperature=model_config.temperature, top_p=model_config.top_p, max_tokens=model_config.max_new_tokens,seed=int(os.environ['RANDOM_SEED']))
    elif model_type == 'anthropic' :
        return ChatAnthropic(model=model_config.name, temperature=model_config.temperature, top_p=model_config.top_p, max_tokens=model_config.max_new_tokens)
