"""
This file is a modified version of the orignal GCG attack implemented by NanoGCG. The original code can be found at https://raw.githubusercontent.com/GraySwanAI/nanoGCG/refs/heads/main/nanogcg/gcg.py. 
Changes are made to add support for multi-prompt optimization.
...
class MCG:
    ...
    def run(
        ...
        self.target_ids = tokenizer([target], add_special_tokens=False, return_tensors="pt")["input_ids"].to(model.device).to(torch.int64)
        ...
        # Verify that the message template contains the optimization string placeholder
        for i,message in enumerate(messages): 
            if "{optim_str}" not in message:
                message = message + "{optim_str}"
                
            formatted = [{"role": "user", "content": message}]
    
            template = tokenizer.apply_chat_template(formatted, tokenize=False, add_generation_prompt=True) 
        ...
        n_replace = max(config.n_replace, config.min_replace)
        for _ in tqdm(range(config.num_steps)):
            ...
            with torch.no_grad():
                ...
                print('Sample ids shape:', sampled_ids.shape)
                
                # Compute loss on all candidate sequences 
                batch_size = new_search_width if config.batch_size is None else config.batch_size
                b_losses = []
                for i,after_embeds in enumerate(self.after_embeds_list):
                    if self.config.use_prefix_cache:
                        ...
                        prefix_cache = self.prefix_cache_list[i]
                    else:
                        input_embeds = torch.cat([
                            self.before_embeds_list[i].repeat(new_search_width, 1, 1),
                            self.embedding_layer(sampled_ids),
                            after_embeds.repeat(new_search_width, 1, 1),
                            self.target_embeds.repeat(new_search_width, 1, 1),
                        ], dim=1)
                    ...
                    loss = find_executable_batch_size(self.compute_candidates_loss, batch_size)(input_embeds,prefix_cache)
                    b_losses.append(loss)

                loss = torch.stack(b_losses).mean(dim=0)
            ...
            n_replace = max(config.min_replace,int(n_replace/2))
    ...
    def init_buffer(self) -> AttackBuffer:
        ...
        for i,after_embeds in enumerate(self.after_embeds_list):
            ...
            init_buffer_losses = find_executable_batch_size(self.compute_candidates_loss, true_buffer_size)(init_buffer_embeds,prefix_cache)
            init_buffer_losses_list.append(init_buffer_losses)
            break
    ...
    def compute_token_gradient(
        ...
        grads = []
        for i in range(len(self.after_embeds_list)):
            ...
            if self.config.use_prefix_cache:
                ...
                input_embeds = torch.cat([self.before_embeds_list[i], optim_embeds, self.after_embeds_list[i], self.target_embeds], dim=1)
            ...
            grads.append(optim_ids_onehot_grad)

        # Create mean gradient accross all samples
...
def run(
    ...
    gcg = MCG(model, tokenizer, config)
