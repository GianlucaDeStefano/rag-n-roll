...
class HotflipAttacker():
    def __init__(self, model,c_model,tokenizer,get_emb,max_seq_length=128,pad_to_max_length=True,per_gpu_eval_batch_size=64,num_adv_passage_tokens=30, num_cand=100,num_iter=30,gold_init=True,early_stop=False,score_function='cos_sim',device='cuda') -> None:
    ...
    def hotflip(self, query,top1_score, adv_text,n=1, device='cuda') -> list:
            ...
            for j in range(n):
                ...
                for it_ in range(self.num_iter):
                    ...
                    for token_to_flip in tqdm(range(len(adv_a))):
                ...
                o = self.tokenizer.decode(adv_passage_ids[0][:self.num_adv_passage_tokens], skip_special_tokens=True, clean_up_tokenization_spaces=False)
                triggers.append(o)
                print(f'Found trigger {o} with score: {best_score}')
            return triggers