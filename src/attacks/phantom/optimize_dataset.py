...
default_retriever_id = "facebook/contriever"
default_llm_id = "meta-llama/Meta-Llama-3-8B-Instruct"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--retriever_id', type=str, default=default_retriever_id, help='ID of the retriever to use for optimization')
    parser.add_argument('--chunk_size', type=int, default=1000, help='Number of documents to optimize per query')
    parser.add_argument('--llm_id', type=str, default=default_llm_id, help='ID of the language model to use for optimization')
    ...
    emb = HuggingFaceEmbeddings(model_name=args.retriever_id,model_kwargs={'device':"cuda:0"})
    ...
    retrieval_opt = HotflipAttacker(model= retrieval_model, tokenizer=retrieval_tokenizer,c_model=retrieval_model,get_emb= get_emb,device='cuda',num_adv_passage_tokens=args.hotflip_n_tokens,max_seq_length=256,num_iter=args.hotflip_steps,gold_init=True)
    ...
    end_start = args.split * split_size
