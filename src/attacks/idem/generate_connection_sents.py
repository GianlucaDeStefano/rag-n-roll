...
def filling_generation(args, model, q_id, q_text, p_ids, prompts, contexts, collection, writer):
    ...
    for p_id in fillings.keys():
        ...
        for k, valid_span in enumerate(fillings[p_id]):
            fp_id = p_id + f'-{k}'
            new_fillings.append([q_id, fp_id, q_text, valid_span])
        df = pd.DataFrame(new_fillings, columns=['query_id', 'doc_id', 'query_text', 'doc_text'])
        df.to_csv(writer,mode='a',index=False,header=False,sep='\t')
...
def main():
    ...
    df = pd.read_csv(args.doc_collection, sep='\t', header=None, names=['doc_id', 'doc_text'])
    for i,row in df.iterrows():
        collection[row['doc_id']] = row['doc_text']
    ...
    df = pd.read_csv(args.query_collection, sep='\t', header=None, names=['query_id', 'query_text'])
    for i,row in df.iterrows():
        qry_collection[row['query_id']] = row['query_text']
