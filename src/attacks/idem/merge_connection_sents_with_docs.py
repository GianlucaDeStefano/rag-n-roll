        ...
        with open(output_nsp_file, 'w') as w:
            for q_id in tqdm(queries):
                ...
                for p_id in connect_sents[q_id]:
                    ...
                    p_sents = [str(s) for s in nlp(p_body).sents]
        ...
        for line in f:
            ...
            if q_id not in top_connect_sents:
                ...
                if p_id in top_connect_sents[q_id]:
                    ...
                    print(f'Note: failed to generate connection sentences to target doc {p_id} for query {q_id}.')
                    w1.write(f'{q_id}\t{p_id}\t{qry_collection[q_id]}\t{collection[p_id]}\n')
                    w2.write(f'{q_id}\t{p_id}\t{qry_collection[q_id]}\t"None"\n')
...
def main():
    ...
    qry_collection_df = pd.read_csv(args.query_collection, sep='\t', header=None)
    for i, row in qry_collection_df.iterrows():
        qry_collection[row[0]] = row[1]
    ...
    docs_collection_df = pd.read_csv(args.doc_collection, sep='\t', header=None)
    for i, row in docs_collection_df.iterrows():
        collection[row[0]] = row[1]


    connect_sents = {}
    connect_sents_df = pd.read_csv(args.connect_sent_file, sep='\t', header=None, names=['q_id', 'sent_id', 'q_text', 'sent_text'])
    ...
    for i, row in connect_sents_df.iterrows():
        q_id = row['q_id']
        sent_id = row['sent_id']
        q_text = row['q_text']
